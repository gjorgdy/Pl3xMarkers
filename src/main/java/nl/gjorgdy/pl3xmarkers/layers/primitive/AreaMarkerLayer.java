...
public class AreaMarkerLayer extends MarkerLayer {
    ...
    public void addPoint(@Language("HTML") String label, int color, int x, int z) {
        boolean added = Pl3xMarkers.areaRepository().addPoint(getWorld().getKey(), label, color, x, z);
        if (added) {
            // reload the area
            AreaEntity area = Pl3xMarkers.areaRepository().getArea(getWorld().getKey(), label, color);
    ...
    public void removePoint(@Language("HTML") String label, int color, int x, int z) {
        boolean removed = Pl3xMarkers.areaRepository().removePoint(getWorld().getKey(), label, color, x, z);
        if (removed) {
            // reload the area
            AreaEntity area = Pl3xMarkers.areaRepository().getArea(getWorld().getKey(), label, color);
            if (area != null) loadArea(area);
